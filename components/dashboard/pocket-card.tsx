...
export function PocketCard({ pocket, compact = false }: PocketCardProps) {
  const spentPercentage = pocket.monthlyAllocation > 0 
    ? calculateProgress(pocket.spentThisMonth, pocket.monthlyAllocation)
    : 0;
  
  const savingsProgress = pocket.targetAmount 
    ? calculateProgress(pocket.currentBalance, pocket.targetAmount)
    : null;

  const remaining = pocket.monthlyAllocation - pocket.spentThisMonth;
  const isSavingsPocket = pocket.type === 'emergency' || pocket.type === 'vacation' || pocket.type === 'investment';
      ...
      <div className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{formatCurrency(pocket.currentBalance)}</p>
          </div>
        </div>
      ...
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-4">
          ...
          {pocket.targetDate && (
            <Badge variant="outline" className="text-xs">
              ...
              {new Date(pocket.targetDate).toLocaleDateString('de-DE', { month: 'short', year: 'numeric' })}
            </Badge>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="text-center py-3 bg-muted/50 rounded-lg">
            <p className="text-3xl font-bold">{formatCurrency(pocket.currentBalance)}</p>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-semibold">{formatCurrency(pocket.monthlyAllocation)}</p>
            </div>
            <div className="text-right">
              <p className="text-muted-foreground">Spent This Month</p>
              <p className="font-semibold">{formatCurrency(pocket.spentThisMonth)}</p>
            </div>
          </div>

          {/* Progress Bar */}
          {isSavingsPocket && pocket.targetAmount ? (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground text-center">
                {formatCurrency(pocket.targetAmount - pocket.currentBalance)} remaining to goal
              </p>
            </div>
          ) : (
            <div className="space-y-2">
